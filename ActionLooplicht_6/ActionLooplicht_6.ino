...
void looplicht_vooruit() {
  ...
  for (k=0; k<125; k++) {
    ...
    //delay(1000);
    if (k<25) Step  = Step*1.1+20;
    if (k>=25 && k<100) ;  // Keep it fast for some time
    if (k>=100 && k<125) Step  = (Step-20)/1.1;
  }
}
...
void fade(void) {
  int j, i, val[6] = {0,0,0,0,0,0}, step[6];

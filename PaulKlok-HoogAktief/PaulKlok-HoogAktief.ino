...
uint8_t posPin[4]       = { 19, 21, 22, 23 };               // pins connected to common of digits, right to left

#define CHARLIE_PINS 4
#define CHARLIE_LEDS (CHARLIE_PINS * (CHARLIE_PINS-1))
uint8_t charliePin[CHARLIE_PINS]   = { 12, 14, 27, 26 };
...
void precalcCharlie(void) {
  ...
  for (i=0; i<CHARLIE_PINS; i++) {
    rawCharlieOff |= (1L<<charliePin[i]);
    for (j=0; j<CHARLIE_PINS; j++) {
      if (i==j) continue;
      rawCharlie[charlieOrder[LED]]   = (1L<<charliePin[i]);
      rawCharlieOE[charlieOrder[LED]] = (1L<<charliePin[i]) | (1L<<charliePin[j]);
      LED++;
    }
  }
}
...
void IRAM_ATTR onClock(void) {
  ...
  ++digit &= 3;                                        // next digit
  WRITE_PERI_REG(GPIO_OUT_W1TS_REG, rawDigit[digit]);  // display new digit
...
// init output pins
  for (i=0; i<7; i++) {
    ...
    digitalWrite(segPin[i], LOW); // segments are off when low
  }
  for (i = 0; i < 4; i++) {
    ...
    gpio_set_drive_capability((gpio_num_t)charliePin[i], GPIO_DRIVE_CAP_3);
    pinMode(charliePin[i], OUTPUT);
    digitalWrite(charliePin[i], LOW);
}

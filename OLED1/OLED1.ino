...
void loop() {
	...
	for (;;) {
		...
		if (!bmp180.measureTemperature()) {
			oled.putString(F("None  "));
		} else {
			do {
				delay(20);
			} while (!bmp180.hasValue());
			oled.putFloat(bmp180.getTemperature(), 1);
			oled.putString(F("\177C"));
		}
		...
		if (!bmp180.measurePressure()) {
			...
			oled.putFloat(PressFilt.NewVal(bmp180.getPressure()/100.0), 1);
			oled.putString(F("hPa"));
		}
		
		oled.setTextXY(4, 0);
		oled.putNumber(Count, 8);
 		oled.putNumber(i++, 4);
		oled.setTextXY(6, 0);
		oled.putNumber(Count2, 8);
	}
}

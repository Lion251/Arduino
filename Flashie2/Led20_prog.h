    ...
    switch (Command) {
      ...
      case SPEED:  // Speed. SpeedVal can be set from 0..31 (logarithmic steps)
        ...
        { switch (Param) {
            ...
            case FADEUP:	FadeN++;	break;
            case FADEDOWN:	FadeN--;	break;
      ...
      default:  // Commands 0..4 are lighting of a number of LEDs from the same color
        if (FadeN) {
		switch (FadeN) {
			case 1:	Fade3((Command<<5) | Param);	break;
			case 2:	Fade4((Command<<5) | Param);	break;
			case 3:	Fade5((Command<<5) | Param);	break;
		}
          if (PINB!=0x1F) {
            byte i;

...
void loop()
{
    ...
    //diffuse();
    if (startIndex>=random8(3,8)) {
      startIndex=0;
      //board[random8(MAX_Y)+BORDER][random8(MAX_X)+BORDER] = ColorFromPalette(currentPalette, random8(), 255, LINEARBLEND);
    }
    ...
    for (int y=1; y<MAX_Y+2*BORDER-1; y++) {
      for (int i=0; i<3; i++) {
        temp[y][x][i] = ( board[y-1][x-1][i]*3/2 +                     2*board[y-1][x][i] + board[y-1][x+1][i]*3/2
                        ...
                        + board[y+1][x-1][i]*3/2 +                     2*board[y+1][x][i] + board[y+1][x+1][i]*3/2) / DIFFUSE_CONSTANT;
      }
    }
  }

...
volatile unsigned	PWM[3];

// I want to have at least 4 kHz PWM, thus 1MHz timer clock after prescaler
...
#else
	#if defined(TCCR1) && (TIMER_TO_USE_FOR_MILLIS == 1)
	  #define MillisTimer_Prescale_Index  (0b001)
	  #define ToneTimer_Prescale_Index    (0b001)
	#elif defined(TCCR1) && (TIMER_TO_USE_FOR_MILLIS == 0)
	  #define MillisTimer_Prescale_Index  (0b001)
	  #define ToneTimer_Prescale_Index    (0b0001)
	#elif defined(TCCR1E) && (TIMER_TO_USE_FOR_MILLIS == 1)
	  #define MillisTimer_Prescale_Index  (0b0001)
	  #define ToneTimer_Prescale_Index    (0b001)
	#elif defined(TCCR1E) && (TIMER_TO_USE_FOR_MILLIS == 0)
	  #define MillisTimer_Prescale_Index  (0b001)
	  #define ToneTimer_Prescale_Index    (0b0001)
...
...rmv */
  ...
  {	static byte	PWM0rem;
	...
	NewVal	= PWM[0] + PWM0rem;
	OCR0A	= ~highByte(NewVal);
	PWM0rem	= lowByte(NewVal);
  }	
...
static byte COM_State;
static byte COM_Reg;

static void COM_CheckStartBit(void) {
	if (PORTB & 0x01) COM_State = 0; 
}

static void COM_Prepare(void) {
	COM_Reg	= 0x80;
}

static void COM_Next(void) {
	// Intentionally empty
}

static void COM_ShiftIn(void) {
...
static void COM_NextBit(void) {
	COM_State	= 3;
}

static void COM_CheckStopBit(void) {
}

typedef void (*VOID_FUNC_VOID)(void);

VOID_FUNC_VOID	COM_FuncList[] = {
	COM_CheckStartBit,
	COM_CheckStartBit,
	COM_Prepare,
	COM_Next,
...
ISR(TIMER1_OVF_vect) {
	static byte	PWM1rem, PWM2rem;
	unsigned	NewVal;

	NewVal		= PWM[1] + PWM1rem;
	OCR1A		= ~highByte(NewVal);
	PWM1rem		= lowByte(NewVal);

	NewVal		= PWM[2] + PWM2rem;
	OCR1B		= ~highByte(NewVal);
	PWM2rem		= lowByte(NewVal);

	VOID_FUNC_VOID	ComFunc	= COM_FuncList[COM_State++];	// Increase COM_State by default
	ComFunc();
}


void HiResPWM(byte channel, unsigned val) {
...
void delay(unsigned long ms) {
	long	EndTime=ms+millis();
	while (EndTime-(long)millis()>0) sleep_cpu();
}

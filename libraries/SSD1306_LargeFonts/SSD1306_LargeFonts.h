...
typedef enum {
	SSD1306_Normal=0,
	SSD1306_Compressed=1,
	SSD1306_Centered=2,
	SSD1306_Autocompress=4,
	SSD1306_Bold=8
} SSD1306Mode;

class SSD1306 {
  public:
    SSD1306(uint8_t Address = 0x78)	{ SSD1306_Address	= Address/2;	}
    uint8_t	Mode;
    void init(void);
    ...
    void beginData(void);
    void sendByte(unsigned char x)	{ Wire.write(x);		}
    void endTransmission(void)		{ Wire.endTransmission();	}

    void sendCommand(unsigned char command);
    void sendData(unsigned char Data);

    void setTextXY(unsigned char Row, unsigned char Column);
    void setWindow(unsigned char x, unsigned char y, unsigned char w, unsigned char h);
    void clearDisplay();
    void setBrightness(unsigned char Brightness);
    void putBar(unsigned char lineNo, unsigned char length);
    bool putChar(unsigned char c);
    void putFlashPascalString(const char *s);
  ...
  private:
    ...
    static byte const PROGMEM InitCode[];
};

extern SSD1306 oled;  // SSD1306 object 

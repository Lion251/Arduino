...
#include "pffs.h"		/* Petit FatFs configurations and declarations */
...
)
{
	...
	if ((UCHAR)*path < ' ') {			/* Null path means the root directory */
		res = dir_rewind(dj);
		FatFs->buf[0] = 0;
...
static
UCHAR check_fs (	/* 0:The FAT boot record, 1:Valid boot record but not an FAT, 2:Not a boot record, 3:Error */
	UCHAR *buf,	/* Working buffer */
...
)
{
	...
	if (ofs > 0) {
		...
		fs->fptr += ofs;
		fs->csect = (UCHAR)(ofs / 512) + 1;	/* Sector offset in the cluster */
